...
/*
 * Open and close
 The open method is provided for a driver to do any initialization in preparation for later operations. In most drivers,
...
int scull_open(struct inode *inode, struct file *filp)
{
	...
	/* "container_of(pointer, container_type, container_field);" this macro takes a pointer to a field of type container_field , within a structure of
	type container_type , and returns a pointer to the containing structure*/

	dev = container_of(inode->i_cdev, struct scull_dev, cdev);// it converts cdev to skull_dev with a cdev field
	...
	if ( (filp->f_flags & O_ACCMODE) == O_WRONLY) {
		if (down_interruptible(&dev->sem))
			return -ERESTARTSYS;
		scull_trim(dev); /* ignore errors */ // this method frees the whole data area if the file is opened for writing
		up(&dev->sem);
	}
	return 0;          /* success */
}
